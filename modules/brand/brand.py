...
#
@brandsblue.route(
    "/brandsearchqueryadd", methods=["GET"], endpoint="brandsearchqueryadd"
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandsearchquerycreate", methods=["POST"], endpoint="brandsearchquerycreate"
)
# @track_emissions
@login_is_required
def brandsearchquerycreate():
    try:
        data = {
            "active": False,
            "queryterm": request.form.get("queryterm"),
...
def brandsearchquery():
    try:
        ...
        if id:
            ...
            return jsonify("{}".format(brandsearchquery.to_dict()["count"])), 200
        else:
            ...
            return render_template(
                "brandsearchquery.html", output=all_brandsearchquerylinks
            )
    except Exception as e:
...
#
@brandsblue.route(
    "/brandsearchqueryedit", methods=["GET"], endpoint="brandsearchqueryedit"
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandsearchquerydelete",
    methods=["GET", "DELETE"],
    endpoint="brandsearchquerydelete",
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandsearchqueryupdate",
    methods=["POST", "PUT"],
    endpoint="brandsearchqueryupdate",
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandsearchqueryactive",
    methods=["GET", "DELETE"],
    endpoint="brandsearchqueryactive",
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandstopwordscreate", methods=["POST"], endpoint="brandstopwordscreate"
)
# @track_emissions
@login_is_required
def brandstopwordscreate():
    try:
        data = {
            "active": False,
            "querykeywords": request.form.get("querykeywords"),
            "language": request.form.get("language"),
        }
...
#
@brandsblue.route(
    "/brandstopwordsdelete", methods=["GET", "DELETE"], endpoint="brandstopwordsdelete"
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandstopwordsupdate", methods=["POST", "PUT"], endpoint="brandstopwordsupdate"
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandstopwordsactive", methods=["GET", "DELETE"], endpoint="brandstopwordsactive"
)
# @track_emissions
@login_is_required
...
def brandlinks():
    try:
        ...
        if id:
            ...
            return jsonify("{}".format(brandlinks.to_dict()["count"])), 200
        else:
            all_brandlinks = []
...
#
@brandsblue.route(
    "/brandlinksdelete", methods=["GET", "DELETE"], endpoint="brandlinksdelete"
)
# @track_emissions
@login_is_required
def brandsearchquerydelete():
...
#
@brandsblue.route(
    "/brandlinksactive", methods=["GET", "DELETE"], endpoint="brandlinksactive"
)
# @track_emissions
@login_is_required
...
def brandmap():
    ...
    # change path to your local directory
    wells_explo = geopandas.read_file("zip://data/wlbPoint.zip", encoding="utf-8")
    ...
    wells_explo_all = wells_explo_sel.loc[
        wells_explo_sel["well_type"].isin(["EXPLORATION"])
    ]

    map_wells = folium.Map(
        location=[
    ...
    fmc = FastMarkerCluster(
        wells_explo_all[
            ["wlbNsDecDeg", "wlbEwDesDeg", "wbName", "age_at_TD"]
        ].values.tolist(),
        callback=callback,
    )
...
#
@brandsblue.route(
    "/brandlinkdetailsadd", methods=["GET"], endpoint="brandlinkdetailsadd"
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandlinkdetailscreate", methods=["POST"], endpoint="brandlinkdetailscreate"
)
# @track_emissions
@login_is_required
...
def brandlinkdetails():
    try:
        ...
        if id:
            ...
            return render_template(
                "brandlinkdetail.html", output=brandlinkdetails.to_dict()
            )
        else:
...
#
@brandsblue.route(
    "/brandlinkdetailsedit", methods=["GET"], endpoint="brandlinkdetailsedit"
)
# @track_emissions
@login_is_required
def brandlinksedit():
    try:
        ...
        brandlinkdetails = brandlinkdetails_ref.document(id).get()
        return render_template(
            "brandlinkdetailsedit.html", ngo=brandlinkdetails.to_dict()
        )
    except Exception as e:
        return f"An Error Occured: {e}"
...
#
@brandsblue.route(
    "/brandlinkdetailsdelete",
    methods=["GET", "DELETE"],
    endpoint="brandlinkdetailsdelete",
)
# @track_emissions
@login_is_required
...
#
@brandsblue.route(
    "/brandlinkdetailsactive",
    methods=["GET", "DELETE"],
    endpoint="brandlinkdetailsactive",
)
# @track_emissions
@login_is_required
